...
Class Shortcodes {
	...
	public static function add_shortcode($tag, $func) {
		...
		if ( is_callable($func, true) )
			static::$shortcode_tags[$tag] = $func;
	}
	...
	public static function has_shortcode( $content, $tag ) {
		if ( static::shortcode_exists( $tag ) ) {
			preg_match_all( '/' . static::get_shortcode_regex() . '/s', $content, $matches, PREG_SET_ORDER );
			if ( empty( $matches ) )
				return false;
	...
	public static function do_shortcode_tag( $m ) {
		...
		if ( isset( $m[5] ) ) {
			...
			return $m[1] . call_user_func( static::$shortcode_tags[$tag], $attr, $m[5], $tag ) . $m[6];
		} else {
			// self-closing tag
			return $m[1] . call_user_func( static::$shortcode_tags[$tag], $attr, null,  $tag ) . $m[6];
		}
	}
